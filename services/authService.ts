...
// File: services/authService.ts  (unify here)
import {
...
// --- Anonymous / Email / Reset ---
export const anonymousLogin = async () => (await signInAnonymously(auth)).user;
...
// --- Google (web popup: lazy; native: id_token) ---
export async function signInWithGoogleWeb() {
  if (Platform.OS !== "web") throw new Error("Web-only Google flow");
  const { GoogleAuthProvider: G, signInWithPopup } = await import(
    "firebase/auth"
  );
...
// --- Phone (web uses invisible recaptcha; native uses modal) ---
...
export async function phoneStart(phoneE164: string, recaptchaRef?: any) {
  if (Platform.OS === "web") {
    const v = ensureWebRecaptcha();
    return signInWithPhoneNumber(auth, phoneE164, v);
  }
  if (!recaptchaRef?.current) throw new Error("Missing reCAPTCHA verifier");
