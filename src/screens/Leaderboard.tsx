...
export default function Leaderboard() {
  ...
  // 🔹 Listen to top users (GLOBAL dataset)
  useEffect(() => {
    const usersQuery = query(
      collection(db, 'users'),
      orderBy('totalPoints', 'desc'),
  ...
  // 🔹 Listen specifically to current user (points + city)
  useEffect(() => {
    const uid = auth.currentUser?.uid;
    if (!uid) {
      setCurrentUserPoints(null);
      setCurrentUserCity(null);
      return;
    }
    ...
    const unsubscribe = onSnapshot(userRef, (snap) => {
      ...
      const total =
        ...
        0;
      setCurrentUserPoints(total);
      setCurrentUserCity((data.city as string | undefined) ?? null);
    });
  ...
  const onRefresh = () => {
    // This just forces onSnapshot to re-run; we don't manually re-query here
    setRefreshing(true);
  };
  ...
  // 🔹 Apply tab filters
  const listForTab: LeaderboardUser[] = (() => {
    if (activeTab === 'global') {
      return users;
    }
    ...
    if (activeTab === 'local') {
      ...
      const cityLower = currentUserCity.toLowerCase();
      return users.filter(
        (u) => u.city && u.city.toLowerCase() === cityLower
      );
    }

    if (activeTab === 'friends') {
      // No friends system yet – keep empty.
      // Later you can populate from cleanupEvents participants etc.
      return [];
    }
  ...
  const topThree = listForTab.slice(0, 3);
  const restUsers = listForTab.slice(3);
  ...
  const renderEmptyState = () => {
    ...
    if (activeTab === 'local') {
      return (
        <View style={styles.emptyState}>
          ...
          >
            Log missions in your area to unlock the local leaderboard.
          </Text>
        </View>
      );
    }
    ...
    return (
      <View style={styles.emptyState}>
        ...
        >
          Invite your squad and track your eco-hero journey together.
        </Text>
      </View>
    );
  };
              ...
              return (
                <Pressable
                  ...
                  style={({ pressed }) => [
                    ...
                    user.uid === auth.currentUser?.uid &&
                      styles.listRowCurrentUser,
                    index < restUsers.length - 1 && styles.listRowDivider,
                    pressed && { opacity: 0.85 },
                  ]}
                >
                  <View style={styles.rankCol}>
